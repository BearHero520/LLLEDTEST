# 绿联 LED 控制工具 - 增强版 v2.1.0

专为绿联 UGREEN 系列 NAS 设备设计的 LED 控制工具，支持 HCTL 智能映射、多 LED 检测、智能硬盘状态监控和**全新的颜色自定义功能**。
...
# 安装完成后，使用主入口命令：
sudo llled-menu          # 🎨 功能菜单 (推荐)
sudo llled-menu color    # 🎨 直接进入颜色配置
sudo llled-menu monitor  # 🤖 启动智能监控
sudo llled-menu status   # 📊 查看当前状态
```
...
-   **预配置服务文件**: 使用经过测试的 systemd 服务配置
-   **分离式安装**: 不再动态创建服务文件，使用现有稳定配置
-   **双模式后台服务**:
    -   **改进版后台服务**: 基于智能硬盘状态显示的稳定监控
    -   **定期状态更新**: 定时调用完整检测功能，稳定性更高
-   **简化安装流程**: 直接使用预下载的服务文件和守护脚本

### 🆕 **v2.1.0 全新颜色自定义功能**

#### 🎨 **完整的 LED 颜色配置系统**

-   **电源键 LED**: 正常/待机/错误状态颜色配置
-   **网络 LED**: 活动/空闲/错误/离线状态颜色配置
-   **硬盘 LED**: 活动/空闲/错误/离线状态颜色配置

#### 🌈 **丰富的颜色选择**

-   **13 种预设颜色**: 红/绿/蓝/白/黄/青/紫/橙/粉/浅绿/天蓝/金色
-   **自定义 RGB**: 支持 0-255 全范围 RGB 颜色自定义
-   **实时预览**: 配置时立即预览 LED 效果
